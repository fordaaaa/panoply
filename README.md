...
| **`/cr-fix`** `<issue-number> [issue-number ...] \| all` | Fixes filed issues on one branch, proves the fix by running it, opens a PR. **This one edits code.** | a branch, commits, a PR |
