<!-- GENERATED by build.mjs from debug.md — do not edit by hand. -->
# debug

> Debug by falsifiable hypothesis with a ledger on disk, so no theory gets tested twice
>
> Portable copy-paste version. Replace `{{ARGUMENTS}}` below with your input ([symptom, or an issue number]).
